 ...
 * linking back to the designated pillar post and 4-5 sibling posts in
 * the same cluster. This is the missing structural piece that lets
 * Google (and AI engines) classify topical authority.
 *
...
export default function RelatedCluster({
  ...
  const siblingSlugs = pillar.posts
    .filter((s) => s !== currentSlug)
    .slice(0, 5)

  // Hydrate slugs against blogContent so we get titles, dates, categories.
  const siblings = siblingSlugs
    .map((slug) => blogContent.find((p) => p.slug === slug))
    .filter(Boolean) as Array<(typeof blogContent)[0]>

  if (siblings.length === 0) return null

  // Designate a "pillar" link target. Until the dedicated pillar URL is
  // built, point at the strongest existing post in the cluster.
  const pillarPost = blogContent.find((p) => p.slug === pillar.pillarSlug)
  const pillarHref = pillarPost ? `${prefix}/blog/${pillarPost.slug}` : null
    ...
    <section className="hair-top bg-bone">
      <div className="container-edge max-w-4xl py-16 md:py-20">
        <div className="flex items-center gap-3 overline text-ash mb-6 hair-draw pb-4">
          ...
          <span>{isKo ? '관련 시리즈' : 'Related cluster'}</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-10 mb-10 md:mb-12 items-end">
          ...
          {pillarHref && pillarPost && currentSlug !== pillar.pillarSlug && (
