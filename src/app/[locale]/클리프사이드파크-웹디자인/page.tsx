...
export async function generateMetadata({ params }: { params: { locale: string } }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      description: 'Web design agency serving Cliffside Park, NJ. Korean-American business specialists.',
...
export default function CliffsideParkWebDesignPage({ params }: { params: { locale: string } }) {
  ...
  const localBusinessSchema = {
    ...
    geo: { '@type': 'GeoCoordinates', latitude: '40.8215', longitude: '-73.9879' },
    areaServed: [
      { '@type': 'City', name: 'Cliffside Park' }, { '@type': 'City', name: 'Fort Lee' },
      { '@type': 'City', name: 'Palisades Park' }, { '@type': 'City', name: 'Fairview' },
  ...
  const faqSchema = {
    ...
    mainEntity: locale === 'ko' ? [
      ...
      { '@type': 'Question', name: '클리프사이드파크 한인 비즈니스 전문 서비스가 있나요?', acceptedAnswer: { '@type': 'Answer', text: '네, 클리프사이드파크와 주변 포트리, 팰팍 한인 비즈니스를 위한 이중언어 웹사이트, 한국어 SEO, 커뮤니티 마케팅 등을 제공합니다.' } },
    ] : [
      ...
      { '@type': 'Question', name: 'Do you specialize in Korean businesses in Cliffside Park?', acceptedAnswer: { '@type': 'Answer', text: 'Yes, we specialize in Korean-American businesses in Cliffside Park, Fort Lee, and Palisades Park. Bilingual websites, Korean SEO, and community marketing.' } },
    ]
  }
        ...
        <section className="pt-32 pb-20 px-4 bg-gradient-to-br from-teal-50 via-white to-blue-50">
          <div className="max-w-7xl mx-auto text-center">
            ...
            <h1 className="text-4xl md:text-6xl font-black mb-6 text-gray-900">
              {locale === 'ko'
                ? <>클리프사이드파크 웹사이트 제작<br /><span className="text-teal-600">한인 비즈니스 전문</span></>
                : <>Cliffside Park Web Design<br /><span className="text-teal-600">Korean Business Experts</span></>}
            </h1>
            <p className="text-xl md:text-2xl text-gray-700 mb-8 max-w-3xl mx-auto">
              {locale === 'ko'
                ? '클리프사이드파크, 포트리, 팰팍 한인 비즈니스를 위한 전문 웹사이트 제작 & 디지털 마케팅. 한영 이중언어, 구글 SEO.'
                : 'Expert web design & digital marketing for Korean-American businesses in Cliffside Park, Fort Lee & Bergen County. Bilingual sites, Google SEO.'}
            </p>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-8">
              <div className="bg-white p-8 rounded-xl shadow-sm">
                ...
                <p className="text-gray-600">{locale === 'ko' ? '클리프사이드파크 한인 고객과 미국 고객 모두를 위한 이중언어 웹사이트. 자동 언어 전환, 한국어 SEO 포함.' : 'Websites optimized for both Korean and American customers in Cliffside Park. Auto language switching, Korean SEO included.'}</p>
              </div>
              <div className="bg-white p-8 rounded-xl shadow-sm">
              ...
              <div className="bg-white p-8 rounded-xl shadow-sm">
                ...
                <p className="text-gray-600">{locale === 'ko' ? '클리프사이드파크, 포트리, 팰팍 한인 커뮤니티 타겟 마케팅. 카카오톡 연동, 한인 미디어 광고.' : 'Targeted marketing for Cliffside Park, Fort Lee & Palisades Park Korean communities. KakaoTalk integration, Korean media ads.'}</p>
              </div>
            </div>
          </div>
        </section>
