...
export async function generateMetadata({ params }: { params: { locale: string } }): Promise<Metadata> {
  ...
  if (locale === 'ko') {
    return {
      title: 'Shopify 비용 감사 — 월 $200-800 절감 보장 | 한인 비즈니스 | ZOE LUMOS',
      description: '대부분 Shopify 스토어는 안 쓰는 앱·구독에 매월 $200-800 과지불. Shopify Expert가 전체 스택 감사 + 절감 보장. 30일 안에 감사비 회수 또는 무료. 한·영 이중언어 상담.',
      keywords: 'shopify 비용 절감, shopify 앱 감사, shopify 비용 절약, 쇼피파이 앱 정리, 쇼피파이 비용 감사, shopify cost audit Korean, shopify app audit, reduce shopify costs Korean business',
      alternates: { canonical: koUrl, languages: { 'x-default': enUrl, en: enUrl, ko: koUrl } },
      openGraph: {
        title: 'Shopify 비용 감사 — 월 $200-800 절감 보장',
        description: 'Shopify Expert가 전체 스택 감사 + 절감 보장. 30일 안에 감사비 회수 또는 무료.',
        url: koUrl,
        siteName: 'ZOE LUMOS',
        locale: 'ko_KR',
        type: 'website',
        images: [{ url: `${baseUrl}/api/og?title=Shopify+%EB%B9%84%EC%9A%A9+%EA%B0%90%EC%82%AC&subtitle=Zoe+Lumos+%C2%B7+Shopify+Expert`, width: 1200, height: 630 }],
      },
  ...
  return {
    title: 'Shopify Cost Audit — Cut $200-$800/mo from Your Stack (Guaranteed) | ZOE LUMOS',
    description: 'Most Shopify stores overpay $200-$800/mo on apps and subscriptions that don\'t drive revenue. Shopify Expert audit cuts what isn\'t working — guaranteed savings or it\'s free. Pay back the audit in 30 days. Bilingual EN/KO.',
    keywords: 'shopify cost audit, shopify app audit, reduce shopify costs, shopify expense reduction, shopify expert nj, shopify developer korean, shopify app review, cut shopify monthly costs, shopify subscription audit, shopify plan optimization',
    alternates: { canonical: enUrl, languages: { 'x-default': enUrl, en: enUrl, ko: koUrl } },
    openGraph: {
      title: 'Shopify Cost Audit — Cut $200-$800/mo (Guaranteed)',
      description: 'Shopify Expert audit cuts overpriced apps + plan tiers. Guaranteed savings or it\'s free. Pay back the audit in 30 days.',
      url: enUrl,
      siteName: 'ZOE LUMOS',
      locale: 'en_US',
      type: 'website',
      images: [{ url: `${baseUrl}/api/og?title=Shopify+Cost+Audit&subtitle=Zoe+Lumos+%C2%B7+Shopify+Expert`, width: 1200, height: 630 }],
    },
    robots: { index: true, follow: true, googleBot: { index: true, follow: true, 'max-image-preview': 'large', 'max-snippet': -1 } },
  }
}
...
const COPY = {
  en: {
    ...
    guaranteeTitle: 'The guarantee',
    guarantee: 'If we cannot identify at least $200/month in real, cancellable cost savings in your Shopify stack within the audit window, the audit is free. No conditions. We have run 23 of these audits across Korean SMB Shopify stores in 2025-2026 and have hit the guarantee on every single one.',
    ...
    pricing: 'Audit only: $499 · Audit + full implementation: $899 (we cancel apps, downgrade plan, negotiate with vendors on your behalf) · Free if we cannot find $200+/month in savings',
    ...
    cta: 'Start my Shopify cost audit',
  },
  ko: {
    eyebrow: '서비스 · Shopify Expert 비용 감사',
    h1Lead: 'Shopify 스택에서',
    h1Sub: '월 $200-800 절감.',
    intro: '대부분 감사하는 Shopify 스토어는 — 네이티브 기능을 중복하는 앱, 성장 안 한 (또는 졸업한) 요금제, 12개월 동안 단 한 건의 매출도 못 만든 구독 도구에 — 과지불 중. Shopify Expert + 한인·미국인 스튜디오. 전체 스택 — 앱, 요금제, 결제 프로세서, 마케팅 도구 — 감사 + 매출 못 만드는 것 정리. 절감 보장 또는 무료.',
    statsTitle: '실제 감사 결과',
    stats: [
      { k: '$3,600-$9,600', v: '일반 연간 절감액' },
    ...
    guaranteeTitle: '보장',
    guarantee: '감사 기간 안에 Shopify 스택에서 실제 해지 가능한 월 $200+ 비용 절감을 식별 못 하면 감사 무료. 조건 X. 2025-2026 한인 SMB Shopify 스토어 23건 감사 — 매 건 보장 달성.',
    ...
    pricingTitle: '가격',
    pricing: '감사만 — $499 · 감사 + 전체 구현 — $899 (대신 앱 해지, 요금제 다운그레이드, 벤더 협상) · 월 $200+ 절감 식별 못 하면 무료',
...
export default function Page({ params }: { params: { locale: string } }) {
  ...
  const serviceSchema = {
    ...
    offers: [
      { '@type': 'Offer', priceCurrency: 'USD', price: '499', name: locale === 'ko' ? '감사만' : 'Audit only', description: locale === 'ko' ? '서면 감사 보고서 + 30분 워크스루' : 'Written audit report + 30-min walkthrough' },
      { '@type': 'Offer', priceCurrency: 'USD', price: '899', name: locale === 'ko' ? '감사 + 구현' : 'Audit + implementation', description: locale === 'ko' ? '감사 + 우리가 앱 해지 · 요금제 다운그레이드' : 'Audit + we cancel apps and downgrade plan tier' },
    ],
    url: pageUrl,
  }
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: locale === 'ko' ? 'Shopify 비용 감사가 정말 절감을 보장하나요?' : 'Does the Shopify cost audit really guarantee savings?',
        acceptedAnswer: {
          '@type': 'Answer',
          text: locale === 'ko'
            ? '네. 월 $200+ 절감 식별 못 하면 감사 무료. 2025-2026 한인 SMB Shopify 스토어 23건 감사 모두 보장 달성. 평균 절감액 월 $400-650, 연 환산 $4,800-$7,800.'
            : 'Yes. If we cannot identify at least $200/month in cancellable savings, the audit is free. We have run 23 Korean SMB Shopify audits in 2025-2026 and hit the guarantee on every single one. Average savings: $400-$650/month = $4,800-$7,800/year.',
        },
      },
      {
        '@type': 'Question',
      ...
      },
      {
        ...
        acceptedAnswer: {
          '@type': 'Answer',
          text: locale === 'ko'
            ? '$499 감사 옵션은 본인 직접 (단순함 — 각 해지마다 3-5분). $899 감사 + 구현 옵션은 대신 모든 해지 실행, 요금제 다운그레이드, 필요 시 벤더 직접 협상.'
            : 'With the $499 audit-only option, you execute the cancellations yourself (simple — 3-5 minutes per cancellation). With the $899 audit + implementation option, we execute everything, downgrade your plan, and negotiate with vendors directly on your behalf.',
        },
      },
      {
        '@type': 'Question',
