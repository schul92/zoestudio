...
export async function generateMetadata({ params }: { params: { locale: string } }): Promise<Metadata> {
  ...
  if (locale === 'ko') {
    return {
      title: '리지필드 웹사이트 제작 | Ridgefield 한인 웹디자인 | ZOE LUMOS',
      description: '리지필드(Ridgefield) 웹사이트 제작 전문. 한인 비즈니스 웹디자인, 홈페이지 제작, 구글 SEO, 한인 마케팅. 버겐카운티 현지 에이전시. 100% 한국어 상담.',
      keywords: '리지필드 웹사이트, Ridgefield 웹디자인, 리지필드 한인 비즈니스, 리지필드 SEO, 버겐카운티 웹사이트, 한인 마케팅 에이전시, 뉴저지 웹사이트 제작',
      openGraph: {
        title: '리지필드 웹사이트 제작 - ZOE LUMOS',
        description: '리지필드 한인 비즈니스를 위한 웹사이트 제작 서비스.',
        url: `${baseUrl}/ko/ridgefield-web-design`,
        siteName: 'ZOE LUMOS', locale: 'ko_KR', alternateLocale: 'en_US', type: 'website',
      },
      alternates: {
        canonical: `${baseUrl}/ko/ridgefield-web-design`,
        languages: { 'x-default': `${baseUrl}/ridgefield-web-design`, 'en': `${baseUrl}/ridgefield-web-design`, 'ko': `${baseUrl}/ko/ridgefield-web-design` },
      },
  ...
  return {
    title: 'Ridgefield Web Design | Korean Business Website NJ | ZOE LUMOS',
    description: 'Ridgefield, NJ web design & digital marketing. Korean-American business website specialists in Bergen County. Local SEO, Google Ads, bilingual websites. Free consultation.',
    keywords: 'Ridgefield web design, Ridgefield NJ website, Korean business Ridgefield, Bergen County web design, small business SEO NJ, Ridgefield digital marketing',
    openGraph: {
      title: 'Ridgefield Web Design - ZOE LUMOS',
      description: 'Web design agency in Ridgefield, NJ. Korean-American business specialists.',
      url: `${baseUrl}/ridgefield-web-design`,
      siteName: 'ZOE LUMOS', locale: 'en_US', alternateLocale: 'ko_KR', type: 'website',
    },
    alternates: {
      canonical: `${baseUrl}/ridgefield-web-design`,
      languages: { 'x-default': `${baseUrl}/ridgefield-web-design`, 'en': `${baseUrl}/ridgefield-web-design`, 'ko': `${baseUrl}/ko/ridgefield-web-design` },
    },
    robots: { index: true, follow: true, googleBot: { index: true, follow: true, 'max-video-preview': -1, 'max-image-preview': 'large' as const, 'max-snippet': -1 } },
  }
    ...
    <>
      ...
      <main className="min-h-screen">
        ...
        <section className="pt-32 pb-20 px-4 bg-gradient-to-br from-purple-50 via-white to-blue-50">
          <div className="max-w-7xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 bg-purple-100 text-purple-800 px-4 py-2 rounded-full mb-6">
              ...
              <span className="font-semibold">{locale === 'ko' ? '리지필드, NJ 현지 에이전시' : 'Local Ridgefield, NJ Agency'}</span>
            </div>
            <h1 className="text-4xl md:text-6xl font-black mb-6 text-gray-900">
              {locale === 'ko'
                ? <>리지필드 웹사이트 제작<br /><span className="text-purple-600">한인 비즈니스 전문</span></>
                : <>Ridgefield Web Design<br /><span className="text-purple-600">Korean Business Experts</span></>}
            </h1>
            <p className="text-xl md:text-2xl text-gray-700 mb-8 max-w-3xl mx-auto">
              {locale === 'ko'
                ? '리지필드, 팰리세이즈파크, 포트리 한인 비즈니스를 위한 전문 웹사이트 제작 & 디지털 마케팅. 한영 이중언어, 구글 SEO.'
                : 'Expert web design & digital marketing for Korean-American businesses in Ridgefield, Palisades Park & Bergen County. Bilingual sites, Google SEO.'}
            </p>
        ...
        <section className="py-20 px-4 bg-gray-50 text-gray-900">
          <div className="max-w-7xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-bold text-center mb-12">
              {locale === 'ko' ? '리지필드 비즈니스 맞춤 서비스' : 'Services for Ridgefield Businesses'}
            </h2>
            <div className="grid md:grid-cols-3 gap-8">
              <div className="bg-white p-8 rounded-xl shadow-sm">
                ...
                <p className="text-gray-600">{locale === 'ko' ? '리지필드 한인 고객과 미국 고객 모두를 위한 이중언어 웹사이트. 자동 언어 전환, 한국어 SEO 포함.' : 'Websites optimized for both Korean and American customers. Auto language switching, Korean SEO included.'}</p>
              </div>
              <div className="bg-white p-8 rounded-xl shadow-sm">
                <Search className="w-10 h-10 text-blue-600 mb-4" />
                <h3 className="text-xl font-bold mb-3">{locale === 'ko' ? '리지필드 로컬 SEO' : 'Ridgefield Local SEO'}</h3>
                <p className="text-gray-600">{locale === 'ko' ? '"리지필드 [업종]" 구글 검색 1페이지 노출. Google My Business 최적화, 로컬 키워드 타겟팅.' : 'Rank page 1 for "Ridgefield [your business]" on Google. GMB optimization, local keyword targeting.'}</p>
              </div>
              <div className="bg-white p-8 rounded-xl shadow-sm">
                ...
                <p className="text-gray-600">{locale === 'ko' ? '리지필드, 팰팍 한인 커뮤니티 타겟 마케팅. 카카오톡 연동, 한인 미디어 광고.' : 'Targeted marketing for Ridgefield & Palisades Park Korean communities. KakaoTalk integration, Korean media ads.'}</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="py-16 px-4 bg-white text-gray-900">
          <div className="max-w-7xl mx-auto">
            <h2 className="text-3xl font-bold text-center mb-8">{locale === 'ko' ? '리지필드 & 주변 지역 서비스' : 'Serving Ridgefield & Surrounding Areas'}</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { en: 'Ridgefield', ko: '리지필드', zip: '07657', highlight: true },
        ...
        <section className="py-20 px-4 bg-purple-600 text-white">
          <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-3xl md:text-4xl font-bold mb-6">
              {locale === 'ko' ? '리지필드 비즈니스를 온라인에서 성장시키세요' : 'Grow Your Ridgefield Business Online'}
            </h2>
            <p className="text-xl mb-8 opacity-90">
              {locale === 'ko' ? '오늘 무료 상담을 예약하세요. 전문 웹디자인으로 고객을 늘리세요.' : 'Book your free consultation today. Attract more customers with expert web design.'}
            </p>
