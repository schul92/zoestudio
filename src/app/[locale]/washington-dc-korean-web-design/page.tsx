...
const cityData = koreanCities.find(c => c.slug === 'washington-dc-korean-web-design')!
...
export async function generateMetadata({ params }: { params: { locale: string } }): Promise<Metadata> {
  ...
  const enUrl = `${baseUrl}/washington-dc-korean-web-design`
  const koUrl = `${baseUrl}/ko/워싱턴DC-웹사이트-제작`

  if (locale === 'ko') {
    return {
      title: `${cityData.city.ko} 한인 웹디자인 | ${cityData.city.en} Korean Web Design | ZOE LUMOS`,
      description: `${cityData.city.ko} 한인 웹디자인 전문 에이전시. 한영 이중언어 웹사이트, 구글 SEO, 카카오톡 연동. ${cityData.neighborhoods.slice(0, 4).map(n => n.ko).join(', ')} 전역 서비스. 100% 한국어 상담.`,
  ...
  return {
    title: `${cityData.city.en} Korean Web Design Agency | ZOE LUMOS`,
    description: `Bilingual Korean-English web design, local SEO & KakaoTalk integration for Korean-American businesses in ${cityData.city.en}. Serving ${cityData.neighborhoods.slice(0, 2).map(n => n.en).join(', ')} & ${cityData.region.en}.`,
